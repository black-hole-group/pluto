 ...
 *********************************************************************** */
{
  ...
  for (i = beg; i <= end; i++) state->flux[i][MXn] += state->press[i];
   ...
   if (g_dir == IDIR){
     for (i = beg; i <= end; i++) {
       VAR_LOOP(nv) state->flux[i][nv] *= grid[IDIR].A[i];
     }
   }else{
     r = fabs(grid[IDIR].x[g_i]);
     for (i = beg; i <= end; i++) {
       VAR_LOOP(nv) state->flux[i][nv] *= r;
     }
   }
  #endif
   ...
   if (g_dir == IDIR){
     for (i = beg; i <= end; i++) {
       VAR_LOOP(nv) {
         ...
         #if CH_SPACEDIM > 1
          state->flux[i][nv] *= grid[JDIR].dV[g_j]/m_dx;
         #endif
     }
   }else{
     for (i = beg; i <= end; i++) {
       VAR_LOOP(nv) {
         state->flux[i][nv] *= grid[JDIR].A[i]*grid[IDIR].x[g_i];
         #if CHOMBO_LOGR == YES
          state->flux[i][nv] *= grid[IDIR].dx[g_i]/m_dx;
         #endif
       }
     }
   }
