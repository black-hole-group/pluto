...
#endif
/* ********************************************************************* */
int UpdateSolution (const Data *d, Riemann_Solver *Riemann, 
...
/* -----------------------------------------------------------------
   ...
   print1 ("! UpdateSolution (ctu_update): ");
...
*/
  #endif
/*
  #if (PARABOLIC_FLUX != 0) && (ENTROPY_SWITCH == YES)
   print1 ("! UpdateSolution (ctu_update): ");
...
/* ---------------------------------------------------- 
  ...
  for (g_dir = 0; g_dir < DIMENSIONS; g_dir++){
    ...
    /* -------------------------------------------------------
      ...
      for ((*in) = indx.beg-1; (*in) <= indx.end+1; (*in)++) {
        VAR_LOOP(nv) state.vh[*in][nv] = d->Vc[nv][k][j][i];
      }
...
*/
   ...
   /* ------------------------------------------
     ...
     /* -------------------------------------------------------- 
      ...
      for ((*in) = indx.beg; (*in) <= indx.end; (*in)++) {
        VAR_LOOP(nv) d->Uc[k][j][i][nv] += state.rhs[*in][nv];
      }           
    }
  }
