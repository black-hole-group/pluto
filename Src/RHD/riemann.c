...
/*  ---------------------------------------------------------------
  ...
  for (i = beg; i <= end; i++) {
    ...
    /*  ----  Transform four-velocity interface
     ...
     gL = 1.0 + EXPAND(vl[VXn]*vl[VXn], + vl[VXt]*vl[VXt], + vl[VXb]*vl[VXb]);
     gR = 1.0 + EXPAND(vr[VXn]*vr[VXn], + vr[VXt]*vr[VXt], + vr[VXb]*vr[VXb]);
     gL = sqrt(gL);
     gR = sqrt(gR);
...
/* ----------------------------------------------------------
   ...
   ---------------------------------------------------------- */
    ...
    #if USE_FOUR_VELOCITY == YES  /*  -- find 4-vel from the solution  -- */
