...
/* -- set pointers to coordinates and grid indices --*/
  ...
  /* ---------------------------------------------------------
    ...
    /* -- stress tensor components (only some needed for flux computation) -- */
       ...
       tau_xx[i] = 0;//2.0*nu1*dxVx + (nu2 - (2.0/3.0)*nu1)*div_v; /*2eta1 dxVx + (eta2 - 2/3 eta1) divV*/
       tau_xy[i] = 0;//nu1*(dyVx + dxVy);   /*eta1 (dyVx + dxVy)*/
    ...
    /* -- stress tensor components (only some needed for flux computation) -- */
      ...
      tau_xx[i] = 0;//2.0*nu1*dxVx + (nu2 - (2.0/3.0)*nu1)*div_v;
      EXPAND(tau_xy[i] = 0;,
             tau_xy[i] = 0;,
             tau_xy[i] = 0;)
      ...
      EXPAND(tau_xz[i] = nu1*(r_1*s_1*dzVx + dxVz) ;   ,
             ...
             tau_yy[i] = 0;/*2.0*nu1*(r_1*dyVy + r_1*vi[VX1])
                        + (nu2 - (2.0/3.0)*nu1)*div_v;
  ...
  /* ---------------------------------------------------------
    ...
    /* -- stress tensor components (only some needed for flux computation) -- */
      ...
      tau_xy[j] = 0;//nu1*(r_1*dyVx + dxVy - r_1*vi[VX2]);
                  /*tau_rthita = eta1(1/r dthitaVr + drVthita -1/r Vthita)*/
      tau_yx[j] = tau_xy[j]; /*tau_thitar*/
      tau_yy[j] = 0;//2.0*nu1*(r_1*dyVy + r_1*vi[VX1])
                 /*tau_thitathita= 2 eta1 (1/r dthitaVthita + 1/r Vr) + (eta2 - 2/3 eta1)divV*/
       #if DIMENSIONS <= 2
       ...
       #if DIMENSIONS <= 2
        EXPAND(tau_zz[j] = 0;,//nu1*r_1*vi[VX1]
               tau_zz[j] = 0;,
               tau_zz[j] = 0;);
       #endif
       #if DIMENSIONS == 3
