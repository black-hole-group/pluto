...
/* ****************************************************************** */
void CharTracingStep(const State_1D *state, double **src,
 ...
 ******************************************************************** */
{
  ...
  static double *a2, *h, **src1;

  if (a2 == NULL){
    src1 = Array_2D(NMAX_POINT, NVAR, double);
...
*/
  ...
  /* -------------------------------------------------------
    ...
    for (nv = NFLX; nv--;   ) {
      vp[nv] += 0.5*delta_t*src[i][nv];
      vm[nv] += 0.5*delta_t*src[i][nv];
    }
