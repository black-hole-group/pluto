...
/*!
  ...
  - Configurations #01-04 are 2D purely hydro;
  - Configurations #05-08 are the corresponding MHD extensions;
  - AMR setups are #04 (for HD) and #08 (for MHD).
  - Configurations #09-10 are 3D MHD (a 3D AMR setup, similar to
    section 5.5 of Mignone et al.(2012), is given by configuration #10).
  ...
  \b References: 
     - Section 5.5 of Mignone et al., ApJS (2012)  198:7 \n
     - Stone & Gardiner, Physics of Fluids (2007), 19
...
{
  #ifdef CTU
   if      (x2 < g_domBeg[JDIR]) x2 = 2.0*g_domBeg[KDIR] - x2;
   else if (x2 > g_domEnd[JDIR]) x2 = 2.0*g_domEnd[KDIR] - x2;
  #endif
